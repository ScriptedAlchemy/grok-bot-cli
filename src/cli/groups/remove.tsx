...
export const config = {
  description: 'Remove a bot from a group.',
  inputJsonSchema: {
...
export default async function groupsRemove({ input }: CliRouteProps<typeof inputSchema>) {
  ...
  return (
    <Agent.Result value={summarize(rec)}>
      <Agent.Text>{`Removed from ${rec.name}. Members: ${rec.memberIds.length}`}</Agent.Text>
    </Agent.Result>
  );
}
