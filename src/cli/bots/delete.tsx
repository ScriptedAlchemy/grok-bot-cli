...
export const config = {
  description: 'Delete a bot or group by id or name.',
  inputJsonSchema: {
...
export default async function botsDelete({ input }: CliRouteProps<typeof inputSchema>) {
  ...
  return (
    <Agent.Result value={summarize(rec)}>
      <Agent.Text>{`Deleted ${rec.isGroup ? 'group' : 'bot'} ${rec.name} (${rec.id})`}</Agent.Text>
    </Agent.Result>
  );
}
