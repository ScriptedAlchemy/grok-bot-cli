...
Install copies the scripts out of the package into durable `~/.codex/bin/` paths, so uninstalling the npm package never leaves Desktop pointing into a deleted checkout. Fail-open is the hard rule: the wrapper `exec`s the real standalone `codex` for every non-`app-server` spawn, and falls through to it whenever the bridge, `python3`, or the socket side is missing or broken — Desktop/Codex keep working with stock behavior. Fully quit and relaunch ChatGPT.app after install (or login) so it inherits `CODEX_CLI_PATH`. LaunchAgent persistence is macOS-first; elsewhere install still writes the wrapper and bridge but leaves `CODEX_CLI_PATH` for you to export.
